...
def extract_features(folder_path, folder, img, img_info):
    ...
    for (key, val) in six.iteritems(results):
        glszm_features[img_info + key] = val


    return glszm_features #, gldm_features # ngtdm_features, glrlm_features, , glszm_features shape_features,
...
def update_rows(row_R, row_L, glszm_featuress): 
    glszm_keys = list(glszm_features.keys())
    glszm_keys.sort()
    if img_info[0] == 'R':
        for key in glszm_keys:
            row_R.append(glszm_features[key])
            if int(folder) == 1:
                row_columns.append(key[2:])
        #print('glszm: ' + str(len(glszm_features)))
    elif img_info[0] == 'L':
        for key in glszm_keys:
            row_L.append(glszm_features[key])
            # if int(folder) == 1:
            #    row_columns.append(key[2:])
    else:
...
if __name__ == "__main__":
        ...
        if folder[0] == '0': 
            ...
            for i, img in enumerate(sorted_imgs):
                ...
                if img_info != None:
                    ...
                    glszm_features = extract_features(folder_path, folder, img, img_info)
                    ...
                    row_R, row_L, row_columns = update_rows(row_R, row_L, glszm_features)
            ...
            print(len(row_R))
            if len(row_R) == 3921: #25032: 3x3 # 35742: 6x6
    ...
    df.to_csv(root_dir + 'raws3x3_GLSZM.csv', index=False)
