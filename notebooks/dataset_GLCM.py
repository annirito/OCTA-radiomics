...
def extract_features(folder_path, folder, img, img_info):
    ...
    return glcm_features #, gldm_features # ngtdm_features, glrlm_features, , glszm_features shape_features,
...
def update_rows(row_R, row_L, glcm_features): 
    glcm_keys = list(glcm_features.keys())
    glcm_keys.sort()
    if img_info[0] == 'R':
        for key in glcm_keys:
            row_R.append(glcm_features[key])
            if int(folder) == 1:
                row_columns.append(key[2:])
        #print('glcm: ' + str(len(glcm_features)))
    elif img_info[0] == 'L':
        for key in glcm_keys:
            row_L.append(glcm_features[key])
            # if int(folder) == 1:
            #    row_columns.append(key[2:])
    else:
...
if __name__ == "__main__":
        ...
        if folder[0] == '0': 
            ...
            for i, img in enumerate(sorted_imgs):
                ...
                if img_info != None:
                    ...
                    glcm_features = extract_features(folder_path, folder, img, img_info)
                    ...
                    row_R, row_L, row_columns = update_rows(row_R, row_L, glcm_features)
            ...
            print(len(row_R))
            if len(row_R) == 5881: #25032: 3x3 # 35742: 6x6
                database.append(row_R)
            else:
                print('NO OJO R ' + str(folder))
            print(len(row_L))
            if len(row_L) == 5881: # 23492 without NGTDM, GLDM, GLRLM # 33992 for 6x6 without NGTDM
    ...
    df.to_csv(root_dir + 'raws3x3_glcm.csv', index=False)
