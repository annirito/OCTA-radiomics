...
def extract_features(folder_path, folder, img, img_info):
    ...
    #glcmFeatures.enableFeatureByName({'Autocorrelation': True, 'Joint Average': True})
    ...
    for (key, val) in six.iteritems(results):
        glcm_features[img_info + key] = val
    ...
    for (key, val) in six.iteritems(results):
        glrlm_features[img_info + key] = val

    return glrlm_features #, gldm_features # ngtdm_features, glrlm, , glszm_features shape_features,
...
def update_rows(row_R, row_L, glrlm_features): 
    glrlm_keys = list(glrlm_features.keys())
    glrlm_keys.sort()
    if img_info[0] == 'R':
        for key in glrlm_keys:
            row_R.append(glrlm_features[key])
            if int(folder) == 1:
                row_columns.append(key[2:])
        #print('glrlm: ' + str(len(glrlm_features)))
    elif img_info[0] == 'L':
        for key in glrlm_keys:
            row_L.append(glrlm_features[key])
            # if int(folder) == 1:
            #    row_columns.append(key[2:])
    else:
...
if __name__ == "__main__":
    ...
    for folder in folders[:596]: # [100:595]  
        if folder == '0062':
            print('FOLDER 0062')
        ...
        if folder[0] == '0': 
            ...
            for i, img in enumerate(sorted_imgs):
                ...
                if img_info != None:
                    ...
                    glrlm_features = extract_features(folder_path, folder, img, img_info)
                    ...
                    row_R, row_L, row_columns = update_rows(row_R, row_L, glrlm_features)

                if i == 1: # here: ??? if i == 1: but why ? -> for column names 
                    row_columns_ok = row_columns

            print(len(row_R))
            if len(row_R) == 3921: #3921: 3x3 # 5601: 6x6
                database.append(row_R)
            else:
                print('NO OJO R ' + str(folder))
            print(len(row_L))
            if len(row_L) == 3921: # 23492 without NGTDM, GLDM, GLRLM # 33992 for 6x6 without NGTDM
    ...
    df.to_csv(root_dir + 'raws6x6_GLRLM.csv', index=False)
